...
export const PlannerView: React.FC<PlannerViewProps> = ({
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      id: 'msg-1',
      sender: 'ai',
      text: 'I can help you plan a trip, find family-friendly spots in Europe, or suggest packing lists for Iceland.',
      timestamp: 'Just now',
    },
  ]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 md:px-8 pt-20 md:pt-10 pb-24 flex flex-col md:flex-row gap-8 h-full min-h-screen">
      ...
      {/* Right Panel: Immersive Itinerary View */}
      <section className="w-full md:w-7/12 lg:w-8/12 flex flex-col gap-6 relative">
        <div className="glass-panel rounded-2xl p-6 md:p-8 min-h-full border border-white/15 shadow-2xl relative">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b border-white/10 pb-6">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="w-10 h-10 rounded-full glass-panel flex items-center justify-center text-on-surface hover:text-primary transition-colors border border-white/20"
          ...
          <button
            onClick={() => onViewOnMap()}
            className="fixed md:absolute bottom-6 right-6 md:bottom-8 md:right-8 bg-primary text-on-primary hover:bg-primary-fixed font-semibold text-xs md:text-sm px-5 py-3 rounded-full flex items-center gap-2 shadow-2xl border border-white/30 hover:scale-105 active:scale-95 z-30"
          >
